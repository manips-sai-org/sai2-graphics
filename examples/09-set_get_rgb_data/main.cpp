	...
	while (graphics->isWindowOpen()) {
		// update graphics robot and object poses in graphics and render
		{
			...
			// if (counter % 60 == 0) {
				...
				auto binary_frame_data = graphics->getFrameBuffer("camera1");  // works from binary 
				// std::FILE* fp = std::fopen("test.bin", "wb");
				// unsigned char* data = &binary_frame_data[0];
				// std::fwrite(data, 4, 96 * 96, fp);
				// std::fclose(fp);
				redis_client.set(CAMERA_FRAME_KEY, binary_frame_data);
			// }
