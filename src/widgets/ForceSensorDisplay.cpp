...
ForceSensorDisplay::ForceSensorDisplay(const std::string& robot_name,
					   const std::string& link_name,
					   const Eigen::Affine3d T_link_sensor,
					   std::shared_ptr<Sai2Model::Sai2Model> robot,
					   chai3d::cWorld* chai_world) : _robot(robot), _link_name(link_name), _T_link_sensor(T_link_sensor)
{
...
void ForceSensorDisplay::update(const Eigen::Vector3d& force_global_frame, const Eigen::Vector3d& moment_global_frame) {
	...
	_display_line_force->m_pointB = chai3d::cVector3d(epointA - force_global_frame*_force_line_scale);
	_display_line_force->setShowEnabled(true);

	// moment:
	_display_line_moment->m_pointA = chai3d::cVector3d(epointA);
	_display_line_moment->m_pointB = chai3d::cVector3d(epointA - moment_global_frame*_moment_line_scale);
