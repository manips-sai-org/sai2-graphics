...
// co-ordinates of 3d space within the capsule's surface using polar representation
struct PointPolar {
...
// NOTE:
// Pose of frame with respect to parent: The origin is located at one end of the capsule's
// defining line segment. The X axis points towards the length of the capsule. Y and Z 
// axes are default, orthogonal to X.
...
public:
     ...
     * @param length Length of the line segment defining the capsule. This is not the
     *        extent of the capsule which equals length + 2* radius.
     * @param mesh_scale User constraint on the ratio of minimum mesh edge length to the
     *        length or diameter of the capsule, whichever is smaller. Controls how fine
     *        the mesh is.
     * @param max_longitudinal_slices Maximum number of mesh slices along length of capsule
     * @param max_circumference_slices Maximum number of mesh slices along single cross 
     *        section arc.
     */
	cCapsule(double radius,
		double length,
		double mesh_scale = 0.1,
...
public:
	...
	void cartesianPoint(Eigen::Vector3d& ret_vector, const PointPolar& point) const;

	// get the projected surface normal of a point given the coordinates in polar form
