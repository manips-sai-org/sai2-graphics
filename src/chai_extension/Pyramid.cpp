...
// ctor
cPyramid::cPyramid(uint num_sides_base,
		double length_base_side,
		double height,
		bool use_base_center_vertex)
: _num_sides_base(num_sides_base), _length_base_side(length_base_side), _height(height),
...
void cPyramid::generateLocalVertexList() {
	...
	for(uint i = 0; i < _num_sides_base; i++) {
		_base_vertices.push_back( AngleAxisd(_included_angle*i, Vector3d::UnitZ()) * vertex0);
	}
}
...
void cPyramid::render(cRenderOptions& a_options)
{
    ...
    if (SECTION_RENDER_PARTS_WITH_MATERIALS(a_options, m_useTransparency))
    {
        ...
        if (!m_displayList.render(m_useDisplayList))
        {
            ...
            if(_f_use_base_center_vertex) {
            	...
            	for(uint i = 0; i < _num_sides_base-1; i++) {
            		glBegin(GL_POLYGON);
						...
						glVertex3d(0, 0, 0);
						glVertex3d(_base_vertices[i][0], _base_vertices[i][1], _base_vertices[i][2]);
						glVertex3d(_base_vertices[i+1][0], _base_vertices[i+1][1], _base_vertices[i+1][2]);
					glEnd();
            	}
            	// add last triangle
            	glBegin(GL_POLYGON);
					glNormal3d(normal[0], normal[1], normal[2]);
					glVertex3d(0, 0, 0);
					glVertex3d(_base_vertices[_num_sides_base-1][0], _base_vertices[_num_sides_base-1][1], _base_vertices[_num_sides_base-1][2]);
					glVertex3d(_base_vertices[0][0], _base_vertices[0][1], _base_vertices[0][2]);
				glEnd();
            } else {
            	Vector3d normal(0,0,-1);
            ...
            // sides
            for(uint i = 0; i < _num_sides_base-1; i++) {
            	Vector3d normal = AngleAxisd(_included_angle*i, Vector3d::UnitZ()) * _normal0;
            	glBegin(GL_POLYGON);
					glNormal3d(normal[0], normal[1], normal[2]);
            ...
            // last triangle
            {
            	Vector3d normal = AngleAxisd(_included_angle*(_num_sides_base-1), Vector3d::UnitZ()) * _normal0;
            	glBegin(GL_POLYGON);
					glNormal3d(normal[0], normal[1], normal[2]);
					glVertex3d(_apex[0], _apex[1], _apex[2]);
					glVertex3d(_base_vertices[_num_sides_base-1][0], _base_vertices[_num_sides_base-1][1], _base_vertices[_num_sides_base-1][2]);
